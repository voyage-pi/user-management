...
def check_user_favorite_place(user_id:int, place_id:str):
    response = supabase.table("user_places").select("*").eq("user_id", user_id).eq("place_id", place_id).execute()
    if response.data and len(response.data) > 0:
        return {"is_saved": True}
    else:
        return {"is_saved": False}
    
def add_user_favorite_place(user_id:int, place_id:str):
...
def remove_user_favorite_place(user_id:int, place_id:str):
